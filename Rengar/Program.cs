...
namespace Rengar
{
    internal class Program
    {
                ...
                try
                {
                    ...
                    KeyBindings.AddItem(
                        new MenuItem("KeysCombo", "Combo").SetValue(
                            new KeyBind(Menu_Orbwalker.Item("Combo_Key").GetValue<KeyBind>().Key, KeyBindType.Press,
                                false)));
                    KeyBindings.AddItem(
                        new MenuItem("KeysMixed", "Harass").SetValue(
                            new KeyBind(Menu_Orbwalker.Item("Harass_Key").GetValue<KeyBind>().Key, KeyBindType.Press,
                                false)));
                    KeyBindings.AddItem(
                    ...
                    KeyBindings.AddItem(
                        new MenuItem("KeysLastHit", "Last Hit").SetValue(
                            new KeyBind(Menu_Orbwalker.Item("LaneFreeze_Key").GetValue<KeyBind>().Key, KeyBindType.Press,
                                false)));
                    KeyBindings.AddItem(
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            // W Range
            if (drawW)
                Render.Circle.DrawCircle(Player.Position, W.Range, W.IsReady() ? Color.Green : Color.DarkRed, 1);

            // E Range
            if (drawE)
                Render.Circle.DrawCircle(Player.Position, E.Range, E.IsReady() ? Color.Green : Color.DarkRed, 1);
            ...
            if (drawES && Config.Item("KeysE").GetValue<KeyBind>().Active)
            {
                ...
                Render.Circle.DrawCircle(SearchPosition, 200f, E.IsReady() ? Color.Green : Color.DarkRed, 1);
            }

            // Ultimate Range
            if (R.Level > 0 && (drawUR.SelectedIndex == 1 || drawUR.SelectedIndex == 3))
                Render.Circle.DrawCircle(Player.Position, 1000f + 1000f*R.Level, Color.Green, 10);
        ...
        private static void OnDraw_EndScene(EventArgs args)
        {
            ...
            if (drawUR.SelectedIndex > 1 && R.Level > 0)
                Render.Circle.DrawCircle(Player.Position, 1000f + 1000f*R.Level, Color.Green, 1, true);
        }
        ...
        {
            if (!Q.IsReady() ||
                !(ActiveMode == Orbwalking.OrbwalkingMode.Combo || ActiveMode == Orbwalking.OrbwalkingMode.LaneClear))
                return false;
